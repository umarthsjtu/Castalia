...
{
	RoutingPacket *netPkt = check_and_cast <RoutingPacket*>(pkt);  //check_and_cast is similar to dynamic_cast used for type change
...
void VirtualRouting::handleMessage(cMessage * msg)
{
	...
	switch (msgKind) {
		...
		case APPLICATION_PACKET:
		{
			 ...
			 * to delete it.
			 */
			fromApplicationLayer(appPacket, appPacket->getAppNetInfoExchange().destination.c_str()); // need to come ture
			return;
		}
		...
		case NETWORK_LAYER_PACKET:
		{
			 ...
			 */
			fromMacLayer(netPacket, info.lastHop, info.RSSI, info.LQI);   //need to come ture
			break;
		}
