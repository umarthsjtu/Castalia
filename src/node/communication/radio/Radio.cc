...
{
	bufferSize = par("bufferSize");   //最大帧数 16
	maxPhyFrameSize = par("maxPhyFrameSize"); //每帧大小 1024 bytes
	PhyFrameOverhead = par("phyFrameOverhead"); // 额外开销 6 bytes
	carrierFreq = par("carrierFreq");  //起始频率 2400 MHz
	collisionModel = (CollisionModel_type) ((int)par("collisionModel")); //四种干扰模型 0 无 1 简单 2 动态 3 复杂
	CCAthreshold = par("CCAthreshold");   // 信号强度 检测信道 高于有干扰
	carrierSenseInterruptEnabled = par("carrierSenseInterruptEnabled");  // 是否夹杂干扰？？ false 
	symbolsForRSSI = par("symbolsForRSSI");  //信号强度标志？ 默认 8
	parseRadioParameterFile(par("RadioParametersFile"));  //读取radio 文件 simulation/parameters/radio

	string startingMode = par("mode");  //无， 默认为读取文件的第一个mode参数
	RXmode = parseRxMode(startingMode);
	trace() << "Initialized RX mode to " << RXmode->name;

	string startingTxPower = par("TxOutputPower");  //默认最高发射
	TxLevel = parseTxLevel(startingTxPower);
	trace() << "Initialized TX power output to " << TxLevel->txOutputPower <<
			" dBm, consuming " << TxLevel->txPowerConsumed << " mW";

	string defaultSleepLevel = par("sleepLevel");  //默认 1.4s
...
void Radio::parseRadioParameterFile(const char *fileName)  //信道文件处理 在 \simulations\parameters\radio
{
