...
#include "banMobilityManager/NodePositions.h"
...
void VirtualMobilityManager::parseDeployment() {
	const char *ct;
	char *c; 
	bool useYourOwnMobilityModule = false;
	...
	nodeLocation.gamma = node->par("gamma");

	// Automatic deployment of the nodes
...
void VirtualMobilityManager::setLocation(double x, double y, double z, double phi, double theta, double gamma, double a)
{
	nodeLocation.x = x;
	nodeLocation.y = y;
	nodeLocation.z = z;
...
void VirtualMobilityManager::notifyWirelessChannel()
{
	...
	positionUpdateMsg->setGamma(nodeLocation.gamma);          // TODO !! : Add this function in wireless channel
	// positionUpdateMsg->setTheta(nodeLocation.acceleration);   // we said no :)
